...
class Yolo_loss(nn.Module):
    def __init__(self, image_size = 608, n_classes=80, n_anchors=3, device=None, batch=2):
...
def train(model, device, config, epochs=5, batch_size=1, save_cp=True, log_step=20, img_scale=0.5):
    ...
    criterion = Yolo_loss(device=device, image_size=config.image_szie, batch=config.batch // config.subdivisions, n_classes=config.classes)
